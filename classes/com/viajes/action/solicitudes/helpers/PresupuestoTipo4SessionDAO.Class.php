...
/**
 * Helper DAO para administrar en sesión los presupuestos tipo 4 de una 
 * solicitud.
 *  
 * @author Marcos
 * @since 05-05-2014
 */
class PresupuestoTipo4SessionDAO extends EntityDAO {
    ...
    public function addEntity( $entity, $idConn=0 ) {
    	...
    	$presupuestosTipo4 = unserialize( $_SESSION[ $this->getVariableSessionName() ] );
    	
    	if( empty($presupuestosTipo4) )
    		$presupuestosTipo4 = new ItemCollection();
    	$this->validateOnAdd($entity);
    	$presupuestosTipo4->addItem($entity);
        
        $_SESSION[$this->getVariableSessionName()] = serialize($presupuestosTipo4);
    ...
    public function deleteEntity($oid, $idConn=0) {
    	...
    	$presupuestosTipo4 = unserialize( $_SESSION[$this->getVariableSessionName()] );
    	$nuevosPresupuestosTipo4 = new ItemCollection();
    	foreach ($presupuestosTipo4 as $oPresupuesto) {
    		...
    		if( $ds_presupuesto != $oid ){
    			$nuevosPresupuestosTipo4->addItem($oPresupuesto);
    		}
    	}
    	
        $_SESSION[$this->getVariableSessionName()] = serialize($nuevosPresupuestosTipo4);
    	
    }

    /**
     * quitamos todos los presupuestosTipo4 de sesión
     */
    ...
    public function getEntitiesCount(CdtSearchCriteria $oCriteria, $idConn=0) {
    	...
    	$presupuestosTipo4 = unserialize($this->getVariableSessionName() );

        return $presupuestosTipo4->size();
    }
    ...
    protected function validateOnAdd(Entity $entity){
    	...
    	/*if((CYTSecureUtils::formatDateToPersist($entity->getDt_fecha_tipo4())<CYTSecureUtils::formatDateToPersist(CYT_RANGO_INI.CYT_PERIODO_YEAR))||(CYTSecureUtils::formatDateToPersist($entity->getDt_fecha_tipo4())>CYTSecureUtils::formatDateToPersist(CYT_RANGO_FIN.((int)CYT_PERIODO_YEAR+1)))){
    			$error .= CYT_MSG_PRESUPUESTO_FECHA_FUERA_RANGO.'<br>';
    			
    		}*/
    	
    	if((CYTSecureUtils::formatDateToPersist($entity->getDt_fecha_tipo4())<CYTSecureUtils::formatDateToPersist(CYT_RANGO_INI.CYT_PERIODO_YEAR))){
